...
## Next session plan
- **Next lesson:** 1/03-matrix-transformations. Warm-up idea: transpose of a column vector (orientation, not reversal) and the shape of `W @ x`. Phase 0 is Done and passed its phase quiz 8/8 (2026-09-24). For Phase 1, default to teaching step by step, intuition-first (the learner asked for it on 1/01). Following the FULL numeric curriculum, not the site's curated path.
